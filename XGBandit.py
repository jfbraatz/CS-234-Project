...
class XGBandit():
    ...
    def fit_xgb(self, x, S):
        ...
        xgb.fit(X, Y)
        return xgb.predict(np.array(x).reshape(1,-1))
    ...
    def predict(self, X_t):
        ...
        for i in self.actions:
            ...
            scores = [self.fit_xgb(X_t, self.T__[j][t-1]) for j in self.actions]
            ...
            pi = max([
                (self.fit_xgb(X_t, self.S[k][t-1]), k) for k in K_hat])[1]
